...
# copyright 2016, 2019 Gilbert Healton
...
# COMMAND:
#	isbare.sh [--quiet | -q]
#       isbare.sh || echo $?;           #show VM type and exit code
#       if vm_type="$(isbare.sh)"; then #capture VM type to string
#           echo bare			#do bare specific code
#       else
#	    vm=$?;			#remember numeric type
#           echo VM="$vm $vm_type";	#do VM specific code
#       fi
#	
...
#     1-199 indicates virtual hosts. See following ISBARE_X_xxx= codes.
...
#	    you really need to, and have tested, VM-vendor specific 
#           behavior is what you need, and are willing to live witn 
#           unexpected changes by vendors beyond control of mere programs
#           along with changes to is_bare.sh itself.
#     200-255 indicates trouble
#
#  NOTES:
#    # Doesn't really consider containers yet, such as Docker. 
#      Maybe exits 190+.
#    # Output text needs to be treated more as an interesting log tidbit 
#      than serious value to be used.
...
ISBARE_X_AWS_EC2=8;	#AWS EC2
ISBARE_X_MS_LINUX=9;	#Microsoft Linux
...
function isbare 
{
    ...
    {   #as some tests are expensive in time, only do once
	...
	if [ -e  /sys/hypervisor/uuid ]; then
	   if grep -q '^ec2' /sys/hypervisor/uuid; then
	      is_virtual_private="aws-ec2";
	      break;		#that's all folks
	   else
	      is_virtual_private="/sys/hypervisor/uuid";
	   fi
	fi

	if uname -a | grep -q 'Microsoft.*Linux'; then
	    is_virtual_private="MS-Linux";
	      break;		#that's all folks
	fi

	typeset dmesg="$( dmesg | sed -e 's/^\[[^]]*\]/./' -e 's/"/@/g' )"
		# slow response: gather up in long string to quickly reuse 
	...
	case "$ibp" in
	   ...
	   (*ec2*)	is_exit_private=$ISBARE_X_AWS_EC2;;
	   (*ms-linux*) is_exit_provate=$ISBARE_X_MS_LINUX;;
